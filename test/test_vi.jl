...
@testset "VI" begin
    ...
    @testset "Gaussian" begin
        ...
        gpgauss = GP(x, y, mZero, kern, l)       #Fit the GP

        Q = vi(gpgauss)

        xtest = range(minimum(gpgauss.x), stop=maximum(gpgauss.x), length=50)
        ...
        for i in 1:nsamps
            visamples[i, :] = rand(gpgauss, xtest, Q)
            push!(ymean, predict_y(gpgauss, xtest)[1])
        end
