...
function predict(gp::GP, x::Matrix{Float64})
    ...
    mu = gp.meanf(x) + distance(x,gp.x,gp.kernel,gp.hyp)*gp.alpha'
    Sigma = distance(x,gp.kernel,gp.hyp) - ((gp.L\distance(x,gp.x,gp.kernel,gp.hyp)')')*(gp.L\distance(gp.x,x,gp.kernel,gp.hyp))
    return (mu, Sigma)
end
