...
push!(cov_procs, KernelTest(LinIso(0.0), d, GaussianProcesses.cov))
push!(cov_procs, KernelTest(LinArd(rand(d)), d, GaussianProcesses.cov))
push!(cov_procs, KernelTest(Noise(0.0), d, GaussianProcesses.cov))
cov_table = run(cov_procs, n)

grad_stack_procs = Array(Proc, 0)
push!(grad_stack_procs, KernelTest(LinIso(0.0), d, GaussianProcesses.grad_stack))
push!(grad_stack_procs, KernelTest(LinArd(rand(d)), d, GaussianProcesses.grad_stack))
